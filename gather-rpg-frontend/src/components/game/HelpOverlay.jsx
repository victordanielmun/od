...
export const HelpOverlay = ({ onClose }) => {
  ...
  const controls = [
    ...
    { keys: ['J', 'Click Izq'], desc: isEs ? 'Ataque Básico (Basic Attack)' : 'Basic Attack' },
    { keys: ['K'], desc: isEs ? 'Combo de Golpes (Physical Combo)' : 'Combo Attack' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm pointer-events-auto p-4 animate-in fade-in duration-300">
      {/* Parchment Box */}
      <div className="relative w-full max-w-3xl bg-[var(--color-parchment)] border-8 border-double border-[var(--color-gold)] p-8 shadow-[0_20px_60px_rgba(0,0,0,0.8)] flex flex-col max-h-[85vh] overflow-hidden animate-in zoom-in-95 duration-300 text-[var(--color-base-dark)]">
        ...
        {/* Header */}
        <div className="flex items-center gap-3 mb-6 pb-4 border-b-2 border-[var(--color-gold-dark)]/30 relative z-10">
          <HelpCircle size={32} className="text-[var(--color-orange-vibrant)]" />
          <h2 className="text-3xl font-medieval uppercase tracking-wider">
            {isEs ? 'Guía del Aventurero' : 'Adventurer\'s Guide'}
          </h2>
          
          <button 
            onClick={onClose} 
            className="absolute top-0 right-0 p-2 bg-[var(--color-orange-vibrant)] text-white border-2 border-[var(--color-gold)] shadow-md hover:bg-[var(--color-base-dark)] transition-colors cursor-pointer active:translate-y-0.5"
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-6 border-b border-gray-300 pb-2 relative z-10">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 font-medieval text-lg uppercase border-2 transition-all cursor-pointer ${
                  isActive
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar-light relative z-10 pr-2">
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {controls.map((ctrl, index) => (
                  <div key={index} className="flex items-center bg-white/40 border border-[var(--color-gold-dark)]/20 p-3 shadow-inner">
                    <div className="flex gap-1.5 mr-4 min-w-[100px] justify-start flex-wrap">
                      {ctrl.keys.map((key, kIdx) => (
                        <kbd 
                          key={kIdx} 
                          className="px-2 py-1 bg-[var(--color-base-dark)] text-[var(--color-gold)] border border-[var(--color-gold-dark)] rounded font-mono text-xs font-bold shadow-md"
                        >
                          {key}
                        </kbd>
                      ))}
                    </div>
                    <span className="text-sm font-serif font-semibold text-gray-800 leading-snug">
                      {ctrl.desc}
                    </span>
                  </div>
                ))}
          ...
          {activeTab === 'gameplay' && (
            <div className="space-y-6 font-serif text-gray-800 animate-in fade-in duration-200">
              <div className="bg-yellow-500/10 border-2 border-dashed border-[var(--color-gold)] p-4 shadow-inner">
                <h4 className="font-medieval text-[var(--color-orange-vibrant)] text-xl uppercase tracking-wider mb-2 flex items-center gap-2">
              ...
              <div>
                <h3 className="text-xl font-medieval text-[var(--color-orange-vibrant)] mb-2 uppercase tracking-wide">
          ...
          {activeTab === 'english' && (
            <div className="space-y-6 font-serif text-gray-800 animate-in fade-in duration-200">
              <div className="bg-white/50 border-l-4 border-[var(--color-orange-vibrant)] p-4 shadow-inner">
                <div className="flex items-center gap-2 text-[var(--color-orange-vibrant)] font-black text-sm uppercase tracking-wide mb-2">
                  <Brain size={16} />
        ...
        {/* Footer */}
        <div className="mt-6 pt-4 border-t border-[var(--color-gold-dark)]/20 text-center relative z-10 flex items-center justify-between">
          <span className="text-xs font-serif text-gray-500 italic flex items-center gap-1">
            <AlertCircle size={12} />
            {isEs ? 'Presiona la tecla "H" para abrir/cerrar esta guía' : 'Press "H" to open/close this guide'}
          </span>
          
          <button
            onClick={onClose}
            className="px-6 py-2 bg-[var(--color-base-dark)] text-[var(--color-gold)] hover:bg-[var(--color-orange-vibrant)] hover:text-white border-2 border-[var(--color-gold)] transition-all font-medieval uppercase text-sm cursor-pointer shadow-lg active:translate-y-0.5"
          >
